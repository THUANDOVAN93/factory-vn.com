...
(function(d, s, id) {
  ...
  fjs.parentNode.insertBefore(js, fjs);
		...
		<ul id="topicpath">
			...
			<?php if ( $this->params['url']['area'] == 'hanoi' ) { ?>
			<li><a href="<?php echo $this->webroot; ?>residence/area/hanoi/">住まいを探す エリアから探す</a></li>
			<?php } else{ ?>
...
<?php foreach(Configure::read('Facility.Residence') as $key => $val) { 
		if ( $residenceBuilding['ResidenceBuilding']['residence_category_id'] != '3' && ($key == "kitchen" || $key == "washer") ) continue;
?>
...
<?php         if (isset($residenceBuilding['ResidenceBuilding']['management_cost']) && !empty($residenceBuilding['ResidenceBuilding']['management_cost'])) { ?>
						<?php echo h(number_format(str_replace(',','',$residenceBuilding['ResidenceBuilding']['management_cost']), 1)); ?>&nbsp;USD/m&sup2;
<?php         } else { ?>
						&nbsp;
<?php         } ?>
					</td>
...
<?php     foreach($residenceBuilding['ResidenceProperty'][2] as $residenceBuildingSale) { ?>
				<tr>
					...
					<td><?php echo h(number_format(str_replace(',','',$residenceBuildingSale['floor_space']))); ?>m&sup2;</td>
					<td><?php echo h(number_format(str_replace(',','',$residenceBuildingSale['price']))); ?></td>
					<td class="note">
...
<?php     foreach($residenceBuilding['ResidenceProperty'][1] as $residenceBuildingRent) { ?>
				<tr>
					...
					<td><?php echo h(number_format(str_replace(',','',$residenceBuildingRent['floor_space']))); ?>m&sup2;</td>
					<td><?php echo h(number_format(str_replace(',','',$residenceBuildingRent['price']))); ?></td>
					<td class="note">
